<#
.SYNOPSIS
	Verifies the ROM file for Final Fantasy II.
...
if ($BuiltRom) {
	...
	$OriginalHash = (Get-FileHash $RomPath -Algorithm SHA256).Hash.ToLower()
	$BuiltHash = (Get-FileHash $BuiltRom -Algorithm SHA256).Hash.ToLower()
