# Build Test Addressing Modes ROM
...
$outputLnx = Join-Path $buildDir "test-addressing-modes.lnx"

Write-Host "Building test-addressing-modes.lnx..." -ForegroundColor Cyan

try {
	poppy build $mainAsm -o $outputLnx --platform lynx --game-name "Addr Modes" --manufacturer "GameGarden"
